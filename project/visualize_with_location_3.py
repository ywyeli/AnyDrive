...
# def create_voxel_grid(pcds, voxel_size=0.1):
#     """
#     根据多帧点云创建体素网格，并根据语义信息上色。
#     :param pcds: 含有多帧点云的列表。
#     :param voxel_size: 体素的尺寸。
#     :return: 上色的体素网格。
#     """
#     # 创建一个体素网格，其中每个体素包含所有帧点云中该位置的颜色平均值
#     colors = []
#     for pcd in pcds:
#         colors.append(np.asarray(pcd.colors))
#     merged_colors = np.mean(colors, axis=0) if colors else None
#
#     # 为了简化，这里我们直接合并点云，实际应用中可能需要根据每个点的语义信息进行更精细的处理
#     merged_pcd = pcds[0]
#     for pcd in pcds[1:]:
#         merged_pcd += pcd
...
#
#     # 体素化处理
#     voxel_grid = o3d.geometry.VoxelGrid.create_from_point_cloud(merged_pcd, voxel_size)
#
#     return voxel_grid
...
def main_visualization():
    ...
    current_scene_index = 271  # Adjust according to your specific scene
    ...
    #
    #     q_translation = [- translation[1], - translation[0], translation[2]]
    ...
    for idx in range(40):
        ...
        rotation = [0, 0, 0]
        transformed_pcd = apply_transformation(pcd, q_translation, rotation)
        pcds.append(transformed_pcd)

    # 创建体素网格并可视化
    voxel_grid = create_voxel_grid(pcds, voxel_size=0.7)
